  ...
  <section style="background:#fff;border-top:1px solid var(--line);border-bottom:1px solid var(--line)">
    <div class="wrap">
      ...
      <div class="grid grid-4">
        ${VEHICLE_CATEGORIES.map(v=>`<div class="card" style="text-align:center"><div style="font-size:26px;margin-bottom:6px">🚗</div><b style="font-size:14px">${v}</b></div>`).join('')}
      </div>
    </div>
  </section>
...
function transparentPricingCards(){
  ...
  return cards.map(c=>`<div class="card"><h3 style="font-size:16px;margin-bottom:4px">${c[0]}</h3><div class="fare" style="margin:6px 0">${c[1]}</div><p style="font-size:13px;color:var(--ink-soft);margin:0">${c[2]}</p></div>`).join('');
}
function heroTrustRow(){
  ...
  if(completed>0 || partners>0){
    return `<div class="hero-stats">
      <div><b>${completed}</b><span>Trips completed</span></div>
      <div><b>${partners}</b><span>Verified partners</span></div>
      <div><b>${DB.settings.serviceAreas.length}</b><span>Service areas</span></div>
    </div>`;
  }
  return `<div class="hero-stats">
    <div><b>✓</b><span>Local booking support</span></div>
    <div><b>✓</b><span>Verified partner network</span></div>
    <div><b>✓</b><span>Clear pricing</span></div>
    <div><b>✓</b><span>Booking status tracking</span></div>
  </div>`;
}
...
function step1Html(d){
  const trips = [
    ['Local Rental','🚕','Hourly or package-based rides within Burdwan.'],
    ['Airport Transfer','✈️','Reliable pickup or drop-off to Kolkata Airport.'],
    ['One Way','➡️','Single pickup to a single drop.'],
    ['Round Trip','↔️','Out and back on a later date.'],
    ['Outstation','🛣️','Longer intercity trips beyond Burdwan.'],
    ['Corporate','💼','Recurring or business travel bookings.']
  ];
  return `
  <div>${trips.map(t=>`<div class="trip-card-lg ${d.tripType===t[0]?'selected':''}" data-act="pick-trip" data-val="${t[0]}"><div class="ic">${t[1]}</div><div><b>${t[0]}</b><span>${t[2]}</span></div></div>`).join('')}</div>
...
function locWarningHtml(){
  ...
  return `<div class="service-area-warn">
    ...
    <div class="row-actions">
      ${s.whatsapp?`<a class="btn btn-accent btn-sm" href="https://wa.me/${s.whatsapp.replace(/\D/g,'')}?text=${encodeURIComponent('Hi, I need a custom quote for a trip outside your listed service area.')}" target="_blank" rel="noopener">WhatsApp us</a>`:''}
...
function step8Html(){
  ...
  <div class="step-actions" style="flex-direction:column">
    ...
    ${s.whatsapp?`<a class="btn btn-accent btn-block" href="https://wa.me/${s.whatsapp.replace(/\D/g,'')}?text=${encodeURIComponent('Hello Burdwan Car Rental, I need help with booking '+bk.id+'.')}" target="_blank" rel="noopener">WhatsApp Support</a>`:''}
  </div>`;
}
...
function reviewFormModal(bookingId){
  ...
  if(!bk) return;
  openModal(`
    <div class="modal-head"><h3>How was your trip?</h3><button class="close-x" data-act="modal-bg-close">✕</button></div>
